...
| **Anthropic Claude API** (Claude 3.5 Sonnet) | Cognitive engine (12s timeout) with contradiction-first templates |
...
| **1:40–2:00 — Zero-downtime fallback** | Call out 8s / 12s `asyncio.wait_for` and the local structured parser (`src/cache.py` + `build_offline_dossier`). Same command still completes a dossier when live keys or networks fail — no crash, no error dump. |
